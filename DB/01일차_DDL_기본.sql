...
create table if not exists 테이블명(
    ...
    # primary key(칼럼명),
    constraint 제약조건명 foreing key(컬럼명) references 참조테이블(기본키),
    # foreing key(컬럼명) references 참조테이블(기본키),
...
# 테이블 수정 - 컬럼 추가
# alter table 테이블명 add 컬럼명 타입 [zerofill] [unique] [not null | null] ...;
alter table student.score add test int not null;
# 테이블 수정 - 컬럼 수정
# alter table 테이블명 change 기존컬러명 새컬럼명 타입 [zerofill] [unique] ...;score
