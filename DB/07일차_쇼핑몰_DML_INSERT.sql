...
INSERT INTO 
MEMBER
	(ME_ID, ME_PW, ME_EMAIL, ME_PHONE, ME_AUTHORITY, ME_FAIL) 
VALUES
	('abc123', 'abc123', 'abc123@naver.com', '011-1234-5678','USER', 0),
	('qwe123', 'qwe123', 'qwe123@naver.com', '011-1111-2222','USER', 0);
    
# 제품 등록하는 쿼리
# 분류 : 옷, 코드 : CLO001, 상품명 : 시원한 티쳐스, 가격 : 20000, 내용 : 티셔츠를 입으면 시원해요.
# 분류 : 옷, 코드 : CLO002, 상품명 : 시원한 반바지, 가격 : 15000, 내용 : 여름 전용 반바지입니다.
# 분류 : 옷, 코드 : CLO003, 상품명 : 시원한 양말, 가격 : 20000, 내용 : 시원한 의류 시리즈 마지막.
INSERT INTO PRODUCT(PR_CODE, PR_NAME, PR_DETAIL, PR_PRICE, PR_CA_NUM)  
SELECT 'CLO001', '시원한 티쳐스', '티셔츠를 입으면 시원해요.', '20000', CA_NUM 
FROM CATEGORY WHERE CA_NAME = '옷';
INSERT INTO PRODUCT(PR_CODE, PR_NAME, PR_DETAIL, PR_PRICE, PR_CA_NUM)  
SELECT 'CLO002', '시원한 반바지', '여름 전용 반바지입니다.', '15000', CA_NUM 
FROM CATEGORY WHERE CA_NAME = '옷';
INSERT INTO PRODUCT(PR_CODE, PR_NAME, PR_DETAIL, PR_PRICE, PR_CA_NUM)  
SELECT 'CLO003', '시원한 양말', '시원한 의류 시리즈 마지막..', '20000', CA_NUM 
FROM CATEGORY WHERE CA_NAME = '옷';

# 분류 : 모자, 코드 : CAP001, 상품명 : 여름 모자, 가격 : 30000, 내용 : 그늘을 만들어줘요.
INSERT INTO PRODUCT(PR_CODE, PR_NAME, PR_DETAIL, PR_PRICE, PR_CA_NUM)  
SELECT 'CAP001', '여름 모자', '그늘을 만들어줘요.', '30000', CA_NUM 
FROM CATEGORY WHERE CA_NAME = '모자';
# 분류 : 신발, 코드 : SHO001, 상품명 : 싼 크록스 가격 : 19999, 내용 : 인기 신발.
INSERT INTO PRODUCT(PR_CODE, PR_NAME, PR_DETAIL, PR_PRICE, PR_CA_NUM)  
SELECT 'SHO001', '싼 크록스', '인기 신발.', '19999', CA_NUM 
FROM CATEGORY WHERE CA_NAME = '신발';
# 분류 : 악세서리, 코드 : ACC001, 상품명 : 금 목걸이 가격 : 100000, 내용 : 부의 상징.
INSERT INTO PRODUCT(PR_CODE, PR_NAME, PR_DETAIL, PR_PRICE, PR_CA_NUM)  
SELECT 'ACC001', '금 목걸이', '부의 상징.', '100000', CA_NUM 
FROM CATEGORY WHERE CA_NAME = '악세서리';

# abc123회원이 CLO001번 제품을 장바구니에 3개 담았을 때 쿼리
INSERT INTO BASKET(BA_AMOUNT, BA_PR_CODE, BA_ME_ID)
SELECT 3, PR_CODE, 'abc123'
FROM PRODUCT WHERE PR_NAME = '시원한 티쳐스';
# abc123회원이 1번 제품을 장바구니에 2개 담았을 때 쿼리
UPDATE BASKET SET BA_AMOUNT = (SELECT BA_AMOUNT + 2) WHERE BA_PR_CODE = 'CLO001' AND BA_ME_ID = 'abc123';

# abc123회원이 ACC001 제품을 장바구니에 1개 담았을 쿼리
INSERT INTO BASKET(BA_AMOUNT, BA_PR_CODE, BA_ME_ID)
VALUES (1, 'ACC001', 'abc123');
# abc123회원이 장바구니에 있는 모든 제품을 구매했을 때 필요한 모든 쿼리
INSERT INTO BUY (BU_PR_CODE, BU_ME_ID, BU_AMOUNT, BU_STATE, BU_DATE)
SELECT BA_PR_CODE, BA_ME_ID, BA_AMOUNT, '구매', NOW()
FROM BASKET WHERE BA_PR_CODE = 'CLO001';
INSERT INTO BUY (BU_PR_CODE, BU_ME_ID, BU_AMOUNT, BU_STATE, BU_DATE)
SELECT BA_PR_CODE, BA_ME_ID, BA_AMOUNT, '구매', NOW()
FROM BASKET WHERE BA_PR_CODE = 'ACC001';
...
INSERT INTO CODE(CO_ME_ID, CO_CODE, CO_TIME) VALUES('abc123', '123asd', DATE_ADD(NOW(), INTERVAL 5 MINUTE));
