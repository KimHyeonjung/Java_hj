...
SELECT LOCATE("안녕1","안녕하세요."); # INSTR / LOCATE 매개변수 위치 차이, 기능은 동일
...
# RGB(255, 20, 3)을 16진수로 변경해서 #FF1403로 표현하는 쿼리
SELECT CONCAT('#', LPAD(HEX(255), 2, '0'), LPAD(HEX(20), 2, '0'), LPAD(HEX(3), 2, '0')) 'RGB(255, 20, 3)을 16진수로';

-- 문자열 교체
# INSERT(문자열, 시작위치, 길이, 대체문자열) : 문자열에서 시작위치부터 길이만큼 문자열을 제거 후 대체문자열을 추가
SELECT INSERT('누구나 하는 C', 8, 1, 'JAVA');
# REPLACE(문자열, 교체할문자열, 대체할문자열)
SELECT REPLACE('C의 정석', 'C', 'JAVA');
# UPPER(문자열) : 문자열을 대문자로 변환, LOWER(문자열) : 문자열을 소문자로 변환
...
# CURRENT_TIMESTAMP는 속성의 타입이 DATETIME인 경우 기본값으로 현재시간을 추가하도록 설정 할 수 있다
SELECT NOW(), SYSDATE(), CURRENT_TIMESTAMP;

# ADDDATE(시간, 차이)/SUBDATE(시간, 차이) : 시간에서 차이만큼 일을 더한/뺀 시간을 계산
SELECT ADDDATE(NOW(), 2), SUBDATE(NOW(), 2);

#ADDTIME(시간, 차이)/SUBTIME(시간, 차이) : 시간에서 차이만큼(시,분,초)를 더한/뺀 시간을 계산
...
SELECT MICROSECOND('2024-07-27 14:10:10.135');
...
SELECT TIMEDIFF('11:00:15', '09:00:5');

# DATE_ADD(시간, INTERVAL 값 유형) : 시간에서 INTERVAL만큼 더한 시간
# DATE_SUB(시간, INTERVAL 값 유형) : 시간에서 INTERVAL만큼 뺀 시간
/*
# INTERVAL 종류
...
# ABS(숫자) : 절대값
SELECT FLOOR(1.24), CEIL(1.23), ROUND(1.53), ABS(-1);
