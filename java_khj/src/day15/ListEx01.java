...
public class ListEx01 {
	...
	public static void main(String[] args) {
		/* 리스트 기본 사용 예제 */
		ArrayList<String> list = new ArrayList<String>(); //크기를 따로 지정해줄 필요가 없다.
		
		/* add(객체) : 객체를 리스트에 뒤에 추가. 컬렉션 인터페이스에서 구현
		 * add(번지, 객체) : 번지 위치에 객체를 추가. 리스트 인터페이스에서 구현 / set은 번지로 접근 불가
		 * */
		list.add("안녕하세요");
		list.add("Hello");
		list.add(0, "Hi");
...
//		list.clear();
		...
		System.out.println("리스트에 홍길동이 있습니까? " + list.contains("고길동"));

		/* indexOf(객체) : 리스트에 객체가 몇번지에 있는지 알려줌. 리스트*/
		System.out.println("리스트에 Hi가 몇번지에 있습니까? " + list.indexOf("Hi"));
		System.out.println("리스트에 홍길동이 몇번지에 있습니까? " + list.indexOf("고길동"));
