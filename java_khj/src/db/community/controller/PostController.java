...
public class PostController {
	...
	public void insertPost(String me_id) {
		//커뮤니티 출력(기본키와 이름을 함께)
		//서비스에게 커뮤니티 리스트를 요청
		List<CommunityVO> cmList = postService.getCommunityList();;
		//커뮤니티 리스트를 이용하여 화면에 출력
		for(CommunityVO community : cmList) {
		...
		PostVO post = new PostVO(coNum, title, content, me_id);
		//서비스에게 게시글VO를 주면서 게시글을 등록하라고 요청
		PrintController.printBar();
		if(postService.insetPost(post)) {
	...
	public PostVO printPostDetail() {
		...
		//서비스에게 조회하려는 게시글의 조회수를 증가시키라고 시킴
