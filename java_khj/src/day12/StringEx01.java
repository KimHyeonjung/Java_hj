...
public class StringEx01 {
	...
	public static void main(String[] args) {
		...
		//String replace(찾을 문자열, 교체할 문자열) : 문자열에서 찾을 문자열을 찾아서 교체할 문자열로 바꾼후 반환
		System.out.println(str + "에서 o를 찾아서 O로 수정 : " + str.replace("o", "O"));
		System.out.println("수정한 후 str : " + str);
		
		//String substring(index) : 시작번지(index) 번지부터 부분 문자열을 추출
		//String substring(index, endIndex) : 시작번지부터 끝번지 전까지 부분 문자열을 추출( 끝 번지 포함 x)
