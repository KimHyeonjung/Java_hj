...
public class StreamEx01 {
	...
	public static void main(String[] args) {
		...
		//다시 사용하려면 list.stream해서 다시 가져와야 된다
		/* map(Function<T,R> f)  : T를 가공해서 R로 변환 시킴
		...
		//등록된 성인 수를 출력
		stream = list.stream();
		long adultCount = stream.filter(p->p.getAge() >= 20).count();
		System.out.println("등록된 성인 수 : " + adultCount);
		
		stream = list.stream();
		//map이 T를 가공해서 R로 만듬. mapToInt는 T를 가공해서 Integer로 만듬
		/* mapToInt는 OptionalDouble로 반환하는데 이유는 객체가 없어서 평균을 구할 수 없으면 
		 * 객체를 통해 알수 있는데 미소드를 제공하기 때문에 사용*/
...
//class A{
//	public static InputStream in = new intputStream() {
//		
//}